...
def CountFile(count_file_in_path, count_analyse, dict_with_entry):
    dict_with_entry = {x: y for x, y in dict_with_entry.items() if y != 0}
    print(f"Всего файлов в проекте = {count_file_in_path}. Проанализированно было {len(count_analyse)}.\n\
В том числе файлы с расширением: {dict_with_entry}")
...
def FindFilesInPath(path):
    ...
    dictEntry = {}  # словарь для анализа колличества проанализированных файлов по расширению
    for i in extensions.list_ex:
        dictEntry[i] = 0  # заполняем словарь информацией из списка
    # цикл поиска файлов в директории и поддиректории
    for folder, subfolders, files in os.walk(path):
        for f in files:
            all_files_count = all_files_count+1
            file_Name, file_Extension = os.path.splitext(
                f)  # разбиваем файл на имя и расширение
            for extension in extensions.list_ex:  # цикл, который ищет все файлы с расширением в list_ex
                if file_Extension == extension and not (file_Name.startswith(".")):
                    ...
                    # Добабвляем в конец списка путь до файла
                    files_in_path.append(folder+"/"+f)
                    dictEntry[extension] = dictEntry[extension]+1
    # Возвращаем список с файлами и колличество всех файлов
    return [files_in_path, all_files_count, dictEntry]
...
# В ListWithPathFile лежат все найденные файлы с расширением из extensions.list_ex. В CountFileInPath колличество всех файлов в папке, в DictEntry словарь с колличеством проанализированных файлов по расширениям.
ListWithPathFiles, CountFilesInPath, DictEntry = FindFilesInPath(
...
# Функция для вывода различной информации
CountFile(CountFilesInPath, ListWithPathFiles, DictEntry)
